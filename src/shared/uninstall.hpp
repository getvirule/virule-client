#pragma once
// Full VIRULE uninstall: removal of every piece of Phase-1 VIRULE-owned
// machine state, driven by an EXPLICIT ownership inventory. Nothing here
// recurses outside VIRULE-owned paths, and nothing here ever touches the
// VIRULE Admin's data (virule.db, workspace\, logs\, backup.json) or an
// Admin-owned machine identity.
//
// The inventory:
//   1. %LOCALAPPDATA%\VIRULE\client\            (recursive; client-only dir)
//   2. security\qa_tester.cred, qa_tester_test.cred (+ .tmp leftovers)
//   3. security\dev_machine.cred ONLY when the client created it
//      (state.json provenance) AND no VIRULE Admin database exists
...
// remove, after the installed process has exited).
inline void remove_owned_state(const std::wstring& installed_exe) {
    // 3 first, while state.json still exists to consult.
    const state::State s = state::load();
    std::error_code ec;
    const bool admin_present = std::filesystem::exists(paths::admin_db_file(), ec) && !ec;
    const auto sec = paths::security_dir();
    if (!sec.empty()) {
        if (s.created_dev_machine_cred && !admin_present) {
            remove_file_quiet(sec / "dev_machine.cred");
            remove_file_quiet(sec / "dev_machine.cred.tmp");
        }
        // 2. QA credentials owned by this installation.
        remove_file_quiet(sec / "qa_tester.cred");
    ...
    // 4. Shared parents only if now empty (never disturb Admin data).
    remove_dir_if_empty(sec);
    remove_dir_if_empty(paths::virule_data_root());

    // 5. virule:// only while it is OURS. A registration the Admin has
    // taken over stays.
    if (protocol_reg::registered_to(installed_exe)) {
        protocol_reg::unregister_protocol();
    }

    // 6. The Apps & Features entry.
...
inline int finish_uninstall(unsigned long parent_pid) {
    if (parent_pid != 0) {
    ...
    remove_owned_state(installed_exe);
...
inline bool spawn_uninstall_helper() {
    wchar_t self[MAX_PATH] = {};
